...
- (void)twk_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self twk_getRangesWithStrings:strings];
...
//    CGPoint point = [touch locationInView:self];
    ...
    [self twk_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(twk_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate twk_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self twk_saveEffectDicWithRange:range];
            
            [self twk_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self twk_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)twk_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self twk_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self twk_getLineBounds:line point:linePoint];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(twk_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)twk_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)twk_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)twk_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)twk_saveEffectDicWithRange:(NSRange)range
{
...
- (void)twk_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf twk_getStringWithRange:range]];
...
- (NSString *)twk_getStringWithRange:(NSRange)range
{
